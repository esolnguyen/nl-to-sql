...
_abstract_type_keys: Dict[str, str] = {
    "api.API": "api_impl",
    "services.db_scanner.Scanner": "db_scanner_impl",
    "databases.mongodb.NlToSQLDatabase": "db_impl",
    "services.context_store.ContextStore": "context_store_impl",
    "databases.vector.VectorDatabase": "vector_store_impl",
}


class Settings(BaseSettings):
    api_impl: str = Field("api.nl_to_sql_api.NLToSQLAPI", env="API_SERVER")
    db_scanner_impl: str = Field(
        "services.db_scanner.SqlAlchemyScannerService", env="DB_SCANNER"
    )
    db_impl: str = Field("databases.mongodb.mongo.MongoDB", env="NL_TO_SQL_DB")
    context_store_impl: str = Field(
        "services.context_store.context_store.ContextStoreService",
        env="CONTEXT_STORE",
    )
    vector_store_impl: str = Field(
        "databases.vector.chroma.Chroma", env="VECTOR_STORE"
    )
