...
from constants.finetuning_prompts import FINETUNING_SYSTEM_INFORMATION
from constants.model_contexts import (
    EMBEDDING_MODEL,
    OPENAI_FINETUNING_MODELS_WINDOW_SIZES,
)
from models.db_description import TableDescription, TableDescriptionStatus
from models.finetuning import FineTuningStatus, Finetuning
...
class OpenAIFineTuning(FinetuningModel):
    ...
    def create_finetuning_dataset(self):
        ...
        for index, example_sql_id in enumerate(self.fine_tuning_model.example_sqls):
            logger.info(
                f"Processing example sql {index + 1} of {len(self.fine_tuning_model.example_sqls)}"
            )
            example_sql = example_sqls_repository.find_by_id(example_sql_id)
            question = example_sql.prompt_text
            query = example_sql.sql
