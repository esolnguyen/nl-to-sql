...
from models.db_connection import DatabaseConnection, DatabaseConnectionRequest
from models.db_description import (
...
from models.instruction import Instruction, InstructionRequest, UpdateInstruction
...
def async_fine_tuning(system, storage, model):
    ...
    openai_fine_tuning.create_finetuning_dataset()
...
class NLToSQLAPI(API):
    def __init__(self, system: System):
        super().__init__(system)
        self.system = system
        self.storage = self.system.instance(NlToSQLDatabase)
    ...
    def add_example_sqls(
        ...
        try:
            created_example_sqls = context_store.add_example_sqls(example_sqls)
        except Exception as e:
            return error_response(
                e,
                {"items": [row.dict() for row in example_sqls]},
                "example_sql_not_created",
            )
        return [
            ExampleSQLResponse(**example_sql.dict())
            for example_sql in created_example_sqls
        ]
    ...
    def delete_example_sql(self, example_sql_id: str) -> dict:
        context_store = self.system.instance(ContextStore)
        status = context_store.remove_example_sqls([example_sql_id])
    ...
    def get_example_sqls(
        ...
        if db_connection_id:
            return example_sqls_repository.find_by(
                {"db_connection_id": str(db_connection_id)},
                page=page,
                limit=limit,
            )
        return example_sqls_repository.find_all(page=page, limit=limit)
    ...
    def create_finetuning_job(
        ...
        try:
            ...
            if fine_tuning_request.example_sqls:
                ...
                if not example_sqls:
                    raise ExampleSQLNotFoundError(
                        f"No example sqls found for db_connection: {fine_tuning_request.db_connection_id}"
                    )
            example_sqls = filter_example_records_based_on_schema(
                example_sqls, fine_tuning_request.schemas
            )
