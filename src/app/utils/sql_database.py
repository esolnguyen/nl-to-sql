...
class SQLDatabase:
    ...
    def get_sql_engine(
        ...
        try:
            db_uri = unquote(fernet_encrypt.decrypt(
                database_info.connection_uri))
    ...
    def from_uri_ssh(cls, database_info: DatabaseConnection):
        ...
        server = SSHTunnelForwarder(
            ...
            ssh_pkey=file_path,
            ssh_private_key_password=fernet_encrypt.decrypt(
    ...
    def get_tables_and_views(self) -> List[str]:
        ...
        if len(rows) == 0:
            raise EmptyDBError(
                "The db is empty it could be a permission issue")
