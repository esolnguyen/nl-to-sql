...
from models.sql_generation import SQLGeneration, SQLGenerationRequest
from repositories.prompts import PromptRepository
from repositories.sql_generations import SQLGenerationNotFoundError, SQLGenerationRepository
from models.prompt import Prompt
from utils.custom_error import EmptySQLGenerationError, PromptNotFoundError, SQLGenerationError
from utils.sql_database import SQLDatabase, create_sql_query_status
...
class SQLGenerationService:
    ...
    def create_sql_generator(self, prompt_id: str, sql_generation_request: SQLGenerationRequest) -> SQLGeneration:
        ...
        if sql_generation_request.sql is not None:
            ...
            try:
                ...
                raise SQLGenerationError(
                    str(e), initial_sql_generation.id) from e
        else:
    ...
    def execute_sql(self, sql_generation_id: str, max_rows: int = 100) -> tuple[str, dict]:
        sql_generation = self.sql_generation_repository.find_by_id(
            sql_generation_id)
    ...
    def _generate_response_with_timeout(self, sql_generator, user_prompt, db_connection, metadata=None):
    ...
    def _update_the_initial_sql_generation(self, initial_sql_generation: SQLGeneration, sql_generation: SQLGeneration):
    ...
    def _get_promp_by_id(self, prompt_id: str, initial_sql_generation: SQLGeneration) -> Prompt:
        prompt = self.prompt_repository.find_by_id(prompt_id)
        if not prompt:
            self._update_error(initial_sql_generation,
    ...
    def _create_init_sql_generation(self, prompt_id: str, llm_config: LLMConfig) -> SQLGeneration:
    ...
    def _create_generator_agent(self, sql_generation_request: SQLGenerationRequest, initial_sql_generation: SQLGeneration) -> SQLGeneration:
        if (
